...
class Blockchain {
  ...
  constructor(config) {
    ...
    this.executionContext.event.register(
      'contextChanged',
      this.resetEnvironment.bind(this)
    );
  ...
  calculateFee(gas, gasPrice, unit) {
    return Web3.utils
      .toBN(gas)
      .mul(
        Web3.utils.toBN(Web3.utils.toWei(gasPrice.toString(10), unit || 'gwei'))
      );
  }
  ...
  async polkadotjs() {
    return this.executionContext.polkadotjs();
  }
      ...
      this.executionContext.detectNetwork((error, network) => {
        ...
        this.txRunner.rawRun(
          ...
          async (error, result) => {
            if (error) return reject(error);
            try {
              const execResult = await this.web3().eth.getExecutionResultFromSimulator(
      ...
      [
        ...
        function getAccount(value, gasLimit, next) {
          ...
          if (self.transactionContextAPI.getAddress) {
            return self.transactionContextAPI.getAddress(function(
              err,
              address
            ) {
          ...
          self.getAccounts(function(err, accounts) {
            ...
            if (
              self.executionContext.isVM() &&
              !self.providers.vm.RemixSimulatorProvider.Accounts.accounts[
                address
              ]
            ) {
        ...
        function runTransaction(fromAddress, value, gasLimit, next) {
          ...
          self.txRunner.rawRun(
            ...
            async (error, result) => {
              ...
              if (isVM && tx.useCall) {
                try {
                  result.transactionHash = await self
                    .web3()
                    .eth.getHashFromTagBySimulator(timestamp);
                } catch (e) {
